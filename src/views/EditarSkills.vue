                ...
                <div class="card h-100">
                  <div class="card-body">
                    <div class="d-flex justify-content-between">
                      ...
                      <div>
                        <button
                          v-if="userAvailableSkillsByNumeroEstudante.length > 0"
                    ...
                    <CompetenceModal
                      ...
                      type="skill"
                      :competences="userAvailableSkillsByNumeroEstudante"
                    ></CompetenceModal>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      userAvailableSkillsByNumeroEstudante:
        "getUserAvailableSkillsByNumeroEstudante",
  ...
  methods: {
    ...
    async competenceNew(skillId, type, percentagem) {
      if (type === "skill") {
        await this.$store.dispatch("AdicionarAlumniLoggedSkillById", {
          id_skills: skillId,
          percentagem: percentagem
        });

        console.log("Add Skill: " + skillId + "  " + percentagem);
      }
    },
    async competenceDeleted(skillId, type) {
      if (type === "skill") {
        await this.$store.dispatch("RemoveAlumniLoggedSkillById", skillId);

        console.log("Remove Skill: " + skillId);
      }
    },
    async competenceChanged(skillId, type, percentagem) {
      if (type === "skill") {
        await this.$store.dispatch("UpdateAlumniLoggedSkillById", {
          id_skills: skillId,
          percentagem: percentagem
        });

        console.log("Skill Changed" + skillId + "  " + "  " + percentagem);
      }
    },
